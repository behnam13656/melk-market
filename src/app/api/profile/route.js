...
export async function POST(req) {
  try {
    ...
    const {
      ...
      category, amenities, rules, images
    } = await req.json();
    ...
    const newProfile = await Profile.create({
      title, description, location, phone, realState,
      constructionDate, amenities, rules, category,
      price: +price, images: images || [], userId: new Types.ObjectId(user._id)
    });
...
export async function PATCH(req) {
  try {
    await connectDB();
    const { _id, title, description, location, phone, realState, price, constructionDate, category, amenities, rules, images } = await req.json();
    ...
    if (!profile) return NextResponse.json({ error: "آگهی یافت نشد" }, { status: 404 });
    if (!user._id.equals(profile.userId)) return NextResponse.json({ error: "دسترسی محدود", status: 403 });
...
export async function DELETE(req) {
  try {
    ...
    if (!_id) return NextResponse.json({ error: "آگهی مشخص نشده" }, { status: 400 });

    const session = await getServerSession(req);
    if (!session) return NextResponse.json({ error: "لطفا وارد حساب کاربری شوید" }, { status: 401 });
    ...
    // حذف تصاویر
    profile.images.forEach(imgUrl => {
      const filePath = path.join(process.cwd(), "public", imgUrl);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    });

    await Profile.deleteOne({ _id });
    return NextResponse.json({ message: "آگهی و تصاویر آن حذف شد" }, { status: 200 });
  } catch (err) {
    console.error(err);
